...
const PRODUCTION_HOSTS: string[] = ["www.butty.london", "butty.london"];
const PRODUCTION_ORIGINS: string[] = PRODUCTION_HOSTS.flatMap((host) => [
  `https://${host}`,
  `http://${host}`,
]);
...
// When BETTER_AUTH_URL is set, still trust the shop's public hosts — the env
// URL is www, but customers also open the apex.
